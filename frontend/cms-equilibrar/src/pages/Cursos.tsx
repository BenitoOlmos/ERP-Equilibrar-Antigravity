...
import { Plus, Edit2, Trash2, Search, GraduationCap, Save, ArrowLeft, PlayCircle, ChevronDown, ChevronUp } from 'lucide-react';
...
export function Cursos() {
  ...
  const fetchAll = () => {
    Promise.all([
       axios.get('/api/master/courses'),
       axios.get('/api/data/services')
    ])
      .then(([resCrs, resServs]) => {
         setCourses(resCrs.data);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`/api/master/courses/${formData.id}`, payload);
      } else {
        await axios.post('/api/master/courses', payload);
      }
      setIsEditing(false);
      fetchAll();
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    if(!confirm('¿Eliminar permanente este Curso de la Academia?')) return;
    try {
  ...
  const toggleActive = async (id: string, currentVal: boolean) => {
    try {
      await axios.patch(`/api/master/courses/${id}/toggle`, { isActive: !currentVal });
      setCourses(prev => prev.map(c => c.id === id ? { ...c, isActive: !currentVal } : c));
    } catch (e) {
      alert('Error updating status');
    }
  };

  const filtered = courses.filter(p => p.title.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <header className="flex items-center justify-between mb-8 shrink-0">
        <div>
          <h1 className="text-3xl font-black text-slate-800 flex items-center">
            <PlayCircle className="w-8 h-8 mr-3 text-orange-500" />
            Academia & Cursos
          </h1>
          <p className="text-slate-500 mt-2">Construya módulos interactivos en video/texto y combínelos opcionalmente con citas de seguimiento clínico en el sistema.</p>
        </div>
        <button 
          onClick={() => { setFormData({ id: '', title: '', description: '', price: '', agendaWeeks: Array.from({length: 4}, (_, i) => ({ weekNumber: i + 1, serviceIds: [] as string[], modules: [] as any[] })) }); setIsEditing(true); }}
          className="bg-orange-500 hover:bg-orange-600 text-white px-5 py-2.5 rounded-xl font-bold flex items-center shadow-lg shadow-orange-500/20 transition-all"
        >
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm flex-1 flex flex-col overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
          <div className="relative w-96">
            ...
            <input 
              type="text" placeholder="Buscar cátedra..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20"
            />
          </div>
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{filtered.length} Cursos</span>
        </div>

        <div className="flex-1 overflow-auto">
          {loading ? (
            <div className="flex justify-center py-20"><div className="w-8 h-8 border-4 border-orange-500/20 border-t-orange-500 rounded-full animate-spin" /></div>
          ) : (
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {filtered.map(prog => (
                  <tr key={prog.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4">
                       <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest bg-slate-50 border border-slate-200 px-2 py-1 rounded w-max">
                          {prog.modules?.length || 0} Módulos / {(prog.services || []).length} Servicios
                       </div>
                    </td>
                    ...
                    <td className="p-4 text-right space-x-2">
                       <button onClick={() => { 
                           ...
                           setFormData({ 
                             id: prog.id, title: prog.title, description: prog.description, price: prog.price?.toString() || '0', 
                             agendaWeeks: aw
                          }); 
                          setIsEditing(true); 
                       }} className="p-2 text-slate-400 hover:text-orange-500 hover:bg-orange-50 rounded-lg transition-colors"><Edit2 className="w-4 h-4" /></button>
                ...
                {filtered.length === 0 && (
                   <tr>
                      <td colSpan={5} className="p-16 text-center text-slate-400 font-bold">
                         <PlayCircle className="w-12 h-12 text-slate-200 mx-auto mb-4" /> No hay Cursos creados
                      </td>
                   </tr>
                )}
              </tbody>
    ...
    <div className="max-w-7xl mx-auto animate-fade-in relative min-h-[calc(100vh-6rem)] flex flex-col">
      <header className="flex items-center justify-between mb-8 shrink-0">
        <div className="flex items-center">
          <button onClick={() => setIsEditing(false)} className="w-10 h-10 bg-white border border-slate-200 rounded-full flex items-center justify-center text-slate-500 hover:text-orange-600 hover:border-orange-200 hover:bg-orange-50 transition-all mr-4 shadow-sm">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h1 className="text-3xl font-black text-slate-800 flex items-center">
              {formData.id ? 'Editor de Cátedra' : 'Aperturar Nuevo Curso'}
            </h1>
            <p className="text-slate-500 mt-2">Confeccione los capítulos y empareje a especialistas para consultas y dudas.</p>
          </div>
        </div>
        <button onClick={handleSave} className="bg-orange-500 text-white px-6 py-2.5 rounded-xl font-black shadow-lg shadow-orange-500/30 hover:bg-orange-600 transition-all flex items-center">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm flex-1 flex flex-col overflow-hidden">
        <div className="p-8 overflow-y-auto flex-1 flex flex-col gap-8 bg-slate-50/50 max-w-4xl mx-auto w-full">
               <div className="space-y-6">
                  <h3 className="font-black text-slate-800 text-sm uppercase tracking-widest border-b border-slate-200 pb-2">Sinopsis</h3>
                  <div className="flex gap-2">
                     <div className="flex-[3]">
                       <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Denominación</label>
                       <input type="text" required value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-4 py-2 font-semibold focus:outline-none focus:border-orange-500" />
                     </div>
                     <div className="flex-1">
                       <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Precio ($)</label>
                       <input type="number" required value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} className="w-full bg-white border border-slate-200 text-orange-600 rounded-xl px-4 py-2 font-black focus:outline-none focus:border-orange-500" />
                     </div>
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Desarrollo Corto</label>
                    <textarea rows={4} value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-orange-500" />
                  </div>
                        ...
                        <div key={wIdx} className="bg-white border border-slate-200 rounded-2xl shadow-sm relative group overflow-hidden">
                           <div className="bg-slate-50 border-b border-slate-200 p-4 flex items-center justify-between cursor-pointer hover:bg-slate-100 transition-colors" onClick={() => toggleWeek(wIdx)}>
                             <div className="flex flex-col">
                               <div className="text-xs font-black uppercase tracking-widest text-orange-500 flex items-center">
                                  <span className="w-5 h-5 rounded bg-orange-500/10 flex items-center justify-center mr-2">W</span> Semana {week.weekNumber}
                               </div>
                           ...
                           {expandedWeeks.includes(wIdx) && (
                              <div className="p-5 space-y-6">
                                 <div>
                                   ...
                                   <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                     {availableServices.map(srv => {
                                        ...
                                        return (
                                           <label key={srv.id} className={`flex items-center px-3 py-2 rounded-xl border cursor-pointer transition-all ${isSel ? 'bg-orange-500/5 border-orange-500/30 shadow-[0_2px_10px_rgba(249,115,22,0.05)]' : 'bg-white border-slate-100/80 hover:bg-slate-50'}`}>
                                              <input type="checkbox" className="mr-3 w-4 h-4 accent-orange-500 rounded border-slate-300" checked={isSel} onChange={() => toggleServiceInWeek(wIdx, srv.id)} />
                                 ...
                                 <div className="bg-slate-50/50 rounded-2xl border border-slate-100 p-4">
                                   <h3 className="font-black text-slate-800 text-xs flex items-center mb-4 uppercase tracking-widest"><PlayCircle className="w-4 h-4 mr-2 text-orange-400" /> Contenidos de la Semana</h3>
        ...
        <div className="p-6 border-t border-slate-100 bg-white flex justify-end gap-3 shrink-0 shadow-[0_-10px_30px_rgba(0,0,0,0.02)] z-10">
          ...
          <button onClick={handleSave} className="px-8 py-2.5 bg-orange-500 text-white font-black rounded-xl shadow-lg shadow-orange-500/30 hover:bg-orange-600 transition-all flex items-center">
            <Save className="w-5 h-5 mr-2" /> Guardar Cambios
          </button>
        </div>
      </div>
